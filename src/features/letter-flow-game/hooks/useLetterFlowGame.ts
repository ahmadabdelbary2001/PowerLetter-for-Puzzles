...
export function useLetterFlowGame() {
  ...
  const handleMouseEnter = useCallback((cell: BoardCell) => {
    ...
    if (cell.letter === activeLetter || cell.letter === '') {
      ...
      if (isTargetEndpoint) {
        ...
        setSelectedPath(prev => [...prev, cell]);

        // Add to found words
        const newFoundWord: WordPath = {
          word: activeLetter,
          cells: [...selectedPath, cell],
          startIndex: 0,
        };
        ...
        setBoard(prev => prev.map(c => 
          [...selectedPath, cell].some(selected => selected.x === c.x && selected.y === c.y)
            ? { ...c, isUsed: true }
            : c
        ));
  ...
  }, [activeLetter, selectedPath, isEndpoint, isCellUsed, foundWords, t, nextLevel, currentLevel]);
