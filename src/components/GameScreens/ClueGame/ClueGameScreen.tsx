...
const ClueGameScreen: React.FC<ClueGameScreenProps> = () => {
  ...
  // Check if language is provided in URL params and update if needed
  useEffect(() => {
  ...
  // Load levels (cast language if your gameUtils expects a different alias)
  useEffect(() => {
    let mounted = true;
    // if lib/gameUtils expects another language type, cast as needed
  ...
  // onHint respects team hint budgets for competitive mode
  const onHint = () => {
    if (gameMode === "competitive" && teams.length > 0) {
  ...
  return (
    <>
      {/* place the page inside the app layout and hide language selector */}
