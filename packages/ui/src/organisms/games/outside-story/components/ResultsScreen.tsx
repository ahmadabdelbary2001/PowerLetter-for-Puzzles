...
export const ResultsScreen: React.FC<Props> = ({ game }) => {
  ...
  if (!currentRound || !currentRound.roundResult) {
    return (
      <div className="text-center">
        ...
        <Button onClick={() => setGameState("round_end")} className="mt-4">
          {t("continue")}
        </Button>
      </div>
    );
  ...
  const { votedPlayerId, outsiderGuessedCorrectly, pointsAwarded } =
  ...
  if (insidersWin) {
    outcomeTitle =
      t("insidersWinTitle", { ns: "outside_the_story" }) || "محققون فازوا!";
    outcomeMessage = t("insidersWin", { ns: "outside_the_story" })?.replace(
      "{secret}",
      currentRound.secret
    );
  } else if (isOutsiderCaught && outsiderGuessedCorrectly) {
    outcomeTitle =
      t("outsiderWinsTitle", { ns: "outside_the_story" }) || "الغريب فاز!";
    outcomeMessage = t("outsiderCaughtButGuessed", {
      ns: "outside_the_story",
    })?.replace("{secret}", currentRound.secret);
  } else {
    outcomeTitle =
      t("outsiderWinsTitle", { ns: "outside_the_story" }) || "الغريب فاز!";
    outcomeMessage = t("outsiderWins", { ns: "outside_the_story" })?.replace(
      "{player}",
      outsider?.name ?? ""
    );
  }
  ...
  return (
    <div className="flex flex-col items-center justify-center w-full max-w-lg mx-auto p-4">
      <div
        className={`w-full p-6 text-center rounded-2xl mb-8 border-2 ${
          insidersWin
        ...
        {players.map((p: Team) => (
          <div
            ...
            className={`flex items-center justify-between p-4 rounded-xl bg-card border border-border shadow-sm ${
              p.id === currentRound.outsiderId ? "ring-2 ring-primary/30" : ""
            }`}
          >
            <div className="flex items-center gap-3">
              ...
              <span className="font-bold text-lg">
                {p.name} {p.id === currentRound.outsiderId && "(🕵️)"}
              </span>
            </div>
