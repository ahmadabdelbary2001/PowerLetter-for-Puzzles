...
import {
  ...
  type LetterFlowLevel,
  type BoardCell,
  type WordPath,
} from "@core/features/games/engine/letter-flow-gameEngine";
import { colorForString } from "@core/shared/lib/colors";
...
export function useLetterFlowGame() {
  ...
  const { currentLevel, gameModeState, nextLevel, setNotification } =
    controller;
  ...
  const allLetters = useMemo(
    () =>
      [
        ...new Set(currentLevel?.endpoints.map((e) => e.letter) || []),
      ] as string[],
    [currentLevel]
  );
  ...
  const findOverlappingConnections = useCallback(
    (newPath: BoardCell[]) => {
      return foundWords.filter((path) =>
        path.cells.some((pc) =>
          newPath.some((nc) => nc.x === pc.x && nc.y === pc.y)
        )
      );
    },
    [foundWords]
  ...
  const handleMouseDown = useCallback(
    (cell: BoardCell) => {
      ...
      const existingConnection = foundWords.find(
        (path) => path.word === cell.letter
      );
      if (existingConnection) {
        setFoundWords((prev) =>
          prev.filter((path) => path.word !== cell.letter)
        );
        setBoard((prev) =>
          prev.map((c) => {
            const used = existingConnection.cells.some(
              (cc) => cc.x === c.x && cc.y === c.y
            );
            return used
              ? {
                  ...c,
                  isUsed: false,
                  color: endpointColorMap.get(`${c.x}-${c.y}`) ?? undefined,
                }
              : c;
          })
        );
        ...
        setNotification({
          messageKey: "connectionRemoved",
          options: { letter: cell.letter },
          type: "info",
          duration: 900,
        });
      }
  ...
  const handleMouseEnter = useCallback(
    (cell: BoardCell) => {
      ...
      const earlierIndex = selectedPath.findIndex(
      ...
      const isTargetEndpoint =
        ...
        !(
          ...
          selectedPath[0].x === cell.x &&
          selectedPath[0].y === cell.y
        ) &&
      ...
      if (isTargetEndpoint) {
        ...
        if (overlapping.length > 0) {
          ...
          setBoard((prev) =>
            prev.map((c) => {
              const overlapped = overlapping.some((op) =>
                op.cells.some((cc) => cc.x === c.x && cc.y === c.y)
              );
              return overlapped
                ? {
                    ...c,
          ...
          setNotification({
            messageKey: "connectionMade",
            options: { letter: activeLetter },
            type: "success",
            duration: 900,
          });
        }
        ...
        const endpointColor =
          board.find((b) => b.x === newPath[0].x && b.y === newPath[0].y)
            ?.color || colorForLetter(newPath[0].letter);
        setBoard((prev) =>
          prev.map((c) => {
            ...
            return used
              ? {
                  ...c,
                  isUsed: true,
                  color: endpointColorMap.get(`${c.x}-${c.y}`) ?? endpointColor,
                }
              : c;
          })
        );
        const newFound: WordPath = {
          word: activeLetter!,
          cells: newPath.map((p) => ({ ...p, color: endpointColor })),
          startIndex: 0,
        };
        setFoundWords((prev) => [
          ...prev.filter((p) => p.word !== activeLetter),
          newFound,
        ]);
        ...
        if (
          allLetters.every((letter: string) => uniqueConnected.includes(letter))
        ) {
          setNotification({
            messageKey: "levelComplete",
            type: "success",
            duration: 2000,
          });
    ...
    if (selectedPath.length > 0) {
      ...
      if (
        ...
        (selectedPath.length > 1 &&
          selectedPath[0].x === lastCell.x &&
          selectedPath[0].y === lastCell.y) ||
  ...
  const onHint = useCallback(() => {
    if (!currentLevel || foundWords.length >= currentLevel.endpoints.length / 2)
      return;
    ...
    setNotification({
      messageKey: "hintRevealed",
      options: { letter: hintLetter },
      type: "info",
      duration: 900,
    });
    // ...
  ...
  const onUndo = useCallback(() => {
    ...
    setBoard((prev) =>
      prev.map((c) => {
        const wasUsed = lastConnection.cells.some(
    ...
    setNotification({
      messageKey: "connectionRemoved",
      options: { letter: lastConnection.word },
      type: "info",
    });
  ...
  const onReset = useCallback(() => {
    ...
    setBoard((prev) =>
      prev.map((c) => ({
        ...c,
        isUsed: false,
        color: endpointColorMap.get(`${c.x}-${c.y}`),
      }))
    );
    setSelectedPath([]);
    setActiveLetter(null);
